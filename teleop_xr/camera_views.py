...
def build_video_streams(camera_views: dict) -> dict:
    ...
    # All cameras are YUYV-only USB cameras; none support 1280x720@30fps.
    ...
    for view_key in order:
        if view_key in camera_views:
            ...
            entry = {"id": view_key, "device": device}
            if view_key == "head":
                entry.update(_HEAD_OVERRIDES)
            elif view_key in ("wrist_left", "wrist_right"):
                entry.update(_WRIST_OVERRIDES)
